...
async function sendMessage() {
  ...
  try {
    if (intent === 'currency') {
      ...
      // No typed intent matched — ask the real Nexora AI (Vercel → OpenRouter).
      const history = chatHistory.slice(-12).map(m => ({ role: m.role, content: m.content }));
      const [aiResult] = await Promise.all([
        askNexoraAI(history),
        new Promise(res => setTimeout(res, 500 + Math.random() * 400)),
      ]);
      if (aiResult && aiResult.ok && aiResult.reply) {
        reply = aiResult.reply;
      } else {
        // Real failure — tell the user the truth so they (and we) can debug.
        const why = (aiResult && aiResult.error) || 'unknown error';
        console.warn('[Nexora] falling back to local reply. Reason:', why);
        const local = generateSmartReply(text);
        reply = `${local}<div class="ai-debug" style="margin-top:8px;font-size:11px;opacity:.55;border-top:1px dashed rgba(255,255,255,.15);padding-top:6px;">⚠ AI offline — ${why}</div>`;
      }
    }
  } catch(err) {
